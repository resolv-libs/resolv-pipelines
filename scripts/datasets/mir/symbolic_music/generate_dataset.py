...
from beam.dofn.mir.symbolic_music.processors import NS_DO_FN_MAP
...
PY_FILE_PATH = str(constants.Paths.SYMBOLIC_MUSIC_BEAM_PIPELINES_DIR / 'generate_dataset_pipeline.py')
...
def _get_beam_pipeline_cmd(config_file_path: Union[str, Path]) -> List[str]:
    generator_config = config.load_configuration_section(config_file_path, 'Main')
    ...
    if processor_config_dir:
        processor_config_dir_path = constants.Paths.SYMBOLIC_MUSIC_DATASET_GENERATOR_CONFIG_DIR / processor_config_dir
        if not processor_config_dir_path.is_dir():
            ...
            processors_flags = beam_utils.beam_options_to_args(processors_configs)
...
if __name__ == '__main__':
    ...
    conf_file_path = constants.Paths.SYMBOLIC_MUSIC_DATASET_GENERATOR_CONFIG_DIR / known_args.config_file_name
