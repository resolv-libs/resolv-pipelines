""" Utility methods for running BEAM pipelines. """
import logging
import select
import shlex
import subprocess
...
def get_runner_args_for_beam_pipeline(config_file_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    """
    main_conf = config.load_configuration_section(config_file_path, 'Runner')
    runner_type = main_conf.get('runner_type')
    if runner_type == 'Direct':
        runner_conf = config.load_configuration_section(config_file_path, constants.BEAMConfigSections.DIRECT_RUNNER)
        ...
        runner_conf = config.load_configuration_section(config_file_path, constants.BEAMConfigSections.FLINK_RUNNER)
        ...
        runner_conf = config.load_configuration_section(config_file_path, constants.BEAMConfigSections.SPARK_RUNNER)
        return beam_options_to_args({
            'runner': 'SparkRunner',
